...
Matrix CoordinateSystem::compute_rodrigues_rotatin_matrix(Coordinate coord, double angle) {
    ...
    double cs1 = 1 - cs;
    // 行列
    ret[0][0] = x * x * cs1 + cs, ret[0][1] = x * y * cs1 - z * sn, ret[0][2] = x * z * cs1 + y * sn;
    ret[1][0] = x * y * cs1 + z * sn, ret[1][1] = y * y * cs1 + cs, ret[1][2] = y * z * cs1 - x * sn;
    ret[2][0] = x * z * cs1 - y * sn, ret[2][1] = y * z * cs1 + x * sn, ret[2][2] = z * z * cs1 + cs;
    return ret;
}

/* LocalCoordinateSystemクラス */
LocalCoordinateSystem::LocalCoordinateSystem() {}
LocalCoordinateSystem::LocalCoordinateSystem(const LocalCoordinateSystem &local_coordinate_system) {
    // 新しいBodyオブジェクトの実体を確保してコピー
    for(auto &&body : local_coordinate_system.bodys) { bodys.push_back(body->clone()); }
}
LocalCoordinateSystem::~LocalCoordinateSystem() {
    // 全てのBodyオブジェクトを解放
...
CameraCoordinateSystem::CameraCoordinateSystem(WorldCoordinateSystem world_coordinate_system, Perspective perspective) {
    ...
    Matrix matrix = CoordinateSystem::compute_affine_transformation_matrix(perspective); //アフィン変換行列
