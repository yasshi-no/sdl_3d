...
LocalCoordinateSystem create_rectangular(double x_length, double y_length,
                                         double z_length)
{
    /* 直方体を生成する.
       原点に頂点を持つ. x, y, z軸に辺を持つ. */
    ...
    ret.add_body(
        new Line(Coordinate(0.0, 0.0, 0.0), Coordinate(x_length, 0.0, 0.0)));
    ret.add_body(new Line(Coordinate(x_length, 0.0, 0.0),
                          Coordinate(x_length, y_length, 0.0)));
    ret.add_body(new Line(Coordinate(x_length, y_length, 0.0),
                          Coordinate(0.0, y_length, 0.0)));
    ret.add_body(
        new Line(Coordinate(0.0, y_length, 0.0), Coordinate(0.0, 0.0, 0.0)));

    ret.add_body(
        new Line(Coordinate(0.0, 0.0, 0.0), Coordinate(0.0, 0.0, z_length)));
    ret.add_body(new Line(Coordinate(x_length, 0.0, 0.0),
                          Coordinate(x_length, 0.0, z_length)));
    ret.add_body(new Line(Coordinate(x_length, y_length, 0.0),
                          Coordinate(x_length, y_length, z_length)));
    ret.add_body(new Line(Coordinate(0.0, y_length, 0.0),
                          Coordinate(0.0, y_length, z_length)));
